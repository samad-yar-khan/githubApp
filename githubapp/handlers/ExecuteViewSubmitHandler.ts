...
export class ExecuteViewSubmitHandler {
	constructor(
		private readonly app: GithubAppApp,
	...
	public async run(context: UIKitViewSubmitInteractionContext) {
		...
		switch (view.id) {
            ...
            case ModalsEnum.ADD_TOKEN_VIEW:
               
                        const { room } = context.getInteractionData();
                        if (room) {
                            const token = view.state?.[ModalsEnum.TOKEN_BLOCK]?.[ModalsEnum.TOKEN_INPUT];
                            console.log("HELLO");
                            await this.setAccessToken(this.read,this.modify,this.http,this.persistence,token,context);
    ...
    private async setAccessToken( read: IRead, modify: IModify, http: IHttp, persis: IPersistence , accessToken :string ,context: UIKitViewSubmitInteractionContext): Promise<void> {
        ...
        await persistence.setUserAccessToken(accessToken, context.getInteractionData().user);
